...
bool QNode::initROSCommunication(std::string grid_map_topic)
{
  ...
  map_subscriber = n.subscribe(grid_map_topic, 1, &QNode::gridMapCallback, this);
...
void QNode::broadcastCurrentWaypoint()
{
  ...
  broadaster.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "/world", "/current_waypoint"));
}
}  // namespace qt_planner
